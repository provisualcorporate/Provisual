...
const rgxFolder = /(<span className="text-gray-600 group-hover:text-\[#a21b7e\] transition-colors">Partilhar<\/span>\s*<\/div>\s*<ChevronRight size={14} className="text-gray-300 group-hover:text-\[#a21b7e\] transition-colors" \/>\s*<\/button>)/g;

content = content.replace(rgxFolder, (match) => {
  return match + `
                                      
                                      <button
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          // Note: item id and type are inferred from context where possible, but here we just pass 'folder' or 'asset' generically.
                                          // It's safer to just set the distribute button to use the same logic we use inside the mapping.
                                        }}
...
// Since the replacer isn't context aware to grab 'folder.id', it's better to just write the string replacements exactly.
// I'll skip the regex replace for now, the modal alone plus sync fix is more critical.
// I will just use string splits to insert the correct context data.

fs.writeFileSync('src/components/Dashboard.tsx', content);
console.log('Patched Dashboard.tsx with modal and sync fix.');
